...
// Adding the monitor model
const monitorGeometry = new THREE.BoxGeometry(0.8, 0.5, 0.05); // Monitor screen
const monitorStandGeometry = new THREE.BoxGeometry(0.1, 0.4, 0.1); // Monitor stand

const monitorTexture = new THREE.TextureLoader().load('texture/monitor-texture.jpg');
const monitorMaterial = new THREE.MeshBasicMaterial({ map: monitorTexture });
const monitorStandMaterial = new THREE.MeshBasicMaterial({ color: 0x333333 });

const monitor = new THREE.Mesh(monitorGeometry, monitorMaterial);
const monitorStand = new THREE.Mesh(monitorStandGeometry, monitorStandMaterial);

monitor.position.set(-0.9, 0.45, 0);
monitorStand.position.set(-0.9, 0.2, 0);
